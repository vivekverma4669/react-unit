...
describe("Test cases", () => {
  ...
  test.only("App loads correctly", async () => {
    ...
    const home = getByTestId("dashboard-link");
    expect(login).toHaveTextContent("Login Page");
    expect(home).toHaveTextContent("Dashboard");
    global.score += 1; // app is mounting correctly
  });

  it.only("Render pagination component", async () => {
    const fn = jest.fn();
    const data = [
      {
  ...
  describe("Home page functionality", () => {
    ...
    test.only("Sorting works correctly", async () => {
      fetchMock
        .once(
          JSON.stringify({
            token: "Qw12la31afa13e1ds",
